...
const calcStrength = (arrCard: string[]): number => {
  ...
  const num5: number = Object.values(countCards).filter(
    (record) => record === 5
  ).length;
  const num4: number = Object.values(countCards).filter(
    (record) => record === 4
  ).length;
  const num3: number = Object.values(countCards).filter(
    (record) => record === 3
  ).length;
  const num2: number = Object.values(countCards).filter(
    (record) => record === 2
  ).length;
  const num1: number = Object.values(countCards).filter(
    (record) => record === 1
  ).length;
